...
Pset = DAVIS_Test(root=DATA_ROOT, imset='example.txt', size='half')
Trainloader = data.DataLoader(Pset, batch_size=1, shuffle=False, num_workers=0, drop_last=True)

# resuming fine-tune
model.load_state_dict(torch.load(os.path.join('cp/weight.pth')))
...
for i, V in tqdm.tqdm(enumerate(Trainloader)):
    ...
    for t in range(2):  # forward : 0, backward : 1
        ...
        for f in index:
            ...
            with torch.no_grad():
                comp = model(rfeats[:, :, ridx], frames[:, :, ridx], masks[:, :, ridx], frames[:, :, f], masks[:, :, f],
                             GTs[:, :, f])
            ...
            save_path = os.path.join('results/', seq_name)
            if t == 1:
    ...
    vid_path = os.path.join('results/', '{}.mp4'.format(seq_name))
    frame_path = os.path.join('results/', seq_name, 'f%d.jpg')
