...
class DAVIS_Test(data.Dataset):
    def __init__(self, root, imset='example.txt', resolution='480p', size=None):
        self.root = root
        self.mask_dir = os.path.join(root, 'results', 'data', 'DAVIS_example', 'JPEGImages', resolution)
        self.image_dir = os.path.join(root, 'results', 'data', 'DAVIS_example', 'JPEGImages', resolution)
        _imset_dir = os.path.join(root, 'data', 'DAVIS_example', 'ImageSets')
        ...
        self.shape = {}
        self.video_target = []
        _video = <type example here>
        self.videos.append(_video)
        self.num_frames[_video] = len(glob.glob(os.path.join(self.image_dir, _video, '*.jpg')))
        print('MASK_DIR', self.mask_dir)
        _mask = np.array(Image.open(os.path.join(self.mask_dir, '{}_mask'.format(_video), '00000.png')).convert("P"))
    ...
    def __getitem__(self, index):
        ...
        for i, f in enumerate(idxs):
            ...
            _video = <type example here>
            mask_file = os.path.join(self.mask_dir, '{}_mask'.format(_video), '{:05d}.png'.format(f))
